...
function() {
	...
	AbstractChosen = function() {
		function AbstractChosen(a, b) {
			...
			AbstractChosen.browser_is_supported() && (this.is_multiple = this.form_field.multiple, this.set_default_text(), this.set_default_values(), this.setup(), this.set_up_html(), this.register_observers(), this.finish_setup())
		}
		...
		AbstractChosen.prototype.results_option_build = function(a) {
			...
			c.group && (c.search_match || c.group_match) ? b += this.result_add_group(c) : !c.empty && c.search_match && (b += this.result_add_option(c)),
		...
		AbstractChosen.prototype.winnow_results = function() {
			...
			for (this.no_results_clear(), d = 0, e = this.get_search_text(), c = this.search_contains ? "": "^", b = new RegExp(c + e.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"), "i"), h = new RegExp(e.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&"), "i"), k = this.results_data, i = 0, j = k.length; j > i; i++) a = k[i],
			a.empty || (a.group && (a.group_match = !1), (!a.group || this.group_search) && (a.search_match = !1, a.search_text = a.group ? a.label: a.html, a.search_match = this.search_string_match(a.search_text, b), a.search_match && (d += 1), a.search_match ? (e.length && (f = a.search_text.search(h), g = a.search_text.substr(0, f + e.length) + "</em>" + a.search_text.substr(f + e.length), a.search_text = g.substr(0, f) + "<em>" + g.substr(f)), null != a.group_array_index && (this.results_data[a.group_array_index].group_match = !0)) : null != a.group_array_index && this.results_data[a.group_array_index].search_match && (a.search_match = !0)));
	...
	a.fn.extend({
		chosen: function(b) {
			return AbstractChosen.browser_is_supported() ? this.each(function() {
				var c;
				return c = a(this),
				c.hasClass("chzn-done") ? void 0 : c.data("chosen", new Chosen(this, b))
			}) : this
		}
	}),
	Chosen = function(d) {
		...
		Chosen.prototype.set_up_html = function() {
			var b, c;
			return b = ["chzn-container"],
			b.push("chzn-container-" + (this.is_multiple ? "multi": "single")),
			this.inherit_select_classes && this.form_field.className && b.push(this.form_field.className),
			this.is_rtl && b.push("chzn-rtl"),
			c = {
				"class": b.join(" "),
				style: "width: " + this.container_width() + ";",
				title: this.form_field.title
			},
			this.form_field.id.length && (c.id = this.form_field.id.replace(/[^\w]/g, "_") + "_chzn"),
			...
			this.is_multiple ? (this.search_choices = this.container.find("ul.chzn-choices").first(), this.search_container = this.container.find("li.search-field").first()) : (this.search_container = this.container.find("div.chzn-search").first(), this.selected_item = this.container.find(".chzn-single").first()),
			this.results_build(),
			this.set_tab_index(),
		...
		Chosen.prototype.register_observers = function() {
			...
			this.search_results.bind("mousewheel DOMMouseScroll",
			function(b) {
		...
		Chosen.prototype.results_build = function() {
			...
			this.results_data = b.SelectParser.select_to_array(this.form_field),
			this.is_multiple ? this.search_choices.find("li.search-choice").remove() : this.is_multiple || (this.single_set_selected_text(), this.disable_search || this.form_field.options.length <= this.disable_search_threshold ? (this.search_field[0].readOnly = !0, this.container.addClass("chzn-container-single-nosearch")) : (this.search_field[0].readOnly = !1, this.container.removeClass("chzn-container-single-nosearch"))),
		...
		Chosen.prototype.set_label_behavior = function() {
			...
			this.form_field_label.length > 0 ? this.form_field_label.click(function(a) {
		...
		Chosen.prototype.single_set_selected_text = function(a) {
			...
			a === this.default_text ? this.selected_item.addClass("chzn-default") : (this.single_deselect_control_build(), this.selected_item.removeClass("chzn-default")),
